...
public class JccRuntime implements RuntimeConst, RTCallbackCaller
{
    public static final int VERSION = 1;

    public static JccRuntime singleton;
    ...
    public void startApp (String entry) throws JccException
    {
        VMThread thread = new VMThread ();
        vThreads.addElement(thread);
        thread.terminateApp = true;
        thread.start(entry, new Object[0]);
    }

    public void startThread (String entry, Object[] args) throws JccException
    {
        VMThread thread = new VMThread ();
        vThreads.addElement(thread);
    ...
    public void stop ()
    {
        for (Enumeration en = vThreads.elements(); en.hasMoreElements(); )
        {
            ((VMThread)en.nextElement()).stop();
        }  
    }
    ...
    class VMThread extends Thread
    {
        ...
        public void start (int entry, Object[] args) throws JccException
        {
            IP = -1;
            vCallStack = new Vector ();
            vStack = new Vector ();
        ...
        {
            try {
                Object ret = library.rtInvokeMethod(iMethod, args, singleton);
        ...
        public void run ()
        {
                ...
                while (bRun)
                {
                    ...
                    switch (opcode)
                    {
                        ...
                        case INVOKE_LIB:
                        {
                            ...
                            for (int i = 0; i < nArgs; i++)
                                args[nArgs-i-1] = pop();
                            if (nArgs==1 && args[0] instanceof String &&
                                    ((String)args[0]).startsWith("found"))
                            {
                                int n123 = 12;
                            }
                        ...
                        {
                            int lib = readByte ();
                            int iObject = readShort ();
                            int size = popInt ();
                            if (lib == -1)
                            {
                                if (iObject == 1)
                                    push (new Object[1][size]);
                                else if (iObject == 2)
                                {
                            ...
                            {
                                Library library = vLibs[lib];
                                Object[] arr =new Object[1];
                                arr[0] = library.rtNewArray(iObject, size);
                            }
                            break;
                        }
                        case NEWARRAY:
                        ...
                        case CTOS:
                        {
                            push (String.valueOf((char)((int[])pop())[0]));
                            break;
                        }
                        case CONCATSTR:
                        {
                        ...
                        case NEWOBJECT:
                        {
                            try {
                                int iLib = readByte ();
                                int iObject = readShort ();
                                Library library = vLibs[iLib];
                                push (library.rtNewObject(iObject));
                            }
                            catch (Exception e)
                            {
                                push (null);
                        ...
                        case STR_TO_INT:
                        {
                            pushInt(Integer.parseInt((String)pop()));
                            break;
                        }
                        case STR_TO_FLOAT:
                        {
                            float[] v = {Float.parseFloat((String)pop())};
                            push (v);
                            break;
                        }
                        default:
            ...
            {
                jcc2.application.Main.singleton.display.setCurrent
                        (jcc2.application.Main.singleton.f);
                jcc2.lib.stdlib.library.singleton.print(e.toString());
                e.printStackTrace();
                vThreads.removeElement(this);
                singleton.stop();
            }
            if (terminateApp)
            {
    	        jcc2.application.Main.singleton.display.setCurrent(jcc2.application.Main.singleton.f);
    			jcc2.application.Main.singleton.f.append("completed");
    			vThreads.removeElement(this);
    			singleton.stop();
            }
        }
    ...
    public static Object allocArray (int dims, int[] lens)
    {
        if (dims == 0)
            return null;
