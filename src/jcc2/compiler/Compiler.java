...
public class Compiler
{
    ...
    public void compileMethod (Node methodNode) throws JccException, IOException
    {
        ...
        int nCallback = ((ASTMethodDeclaration)methodNode).num;
        ...
        if (nCallback == 1)
        {
            methodContainer.bCallback = true;
        }
    ...
    Type compileExpression (MethodContainer method, Node expNode, Type suggestType) throws JccException, IOException
    {
        ...
        {
            Type condType = compileExpression (method, expNode.jjtGetChild(0), null);
            if (!condType.equals (Type.TYPE_BOOL))
                throw new JccException ("" + line + ": condition must be bool");
            Object labNe = generator.reserveLabel();
            Object labEnd = generator.reserveLabel();
            generator.ifeq(labNe); //if zero
            Type tEq = compileExpression (method, expNode.jjtGetChild(1), null);
            generator.jmp(labEnd);
            generator.markLabel(labNe);
            Type tNe = compileExpression (method, expNode.jjtGetChild(2), null);
            generator.markLabel(labEnd);
            if (!tNe.equals(tEq))
                throw new JccException ("" + line + ": incompatible types");
            return tEq;
        ...
        else if (expNode instanceof ASTAdditiveNode)
        {
            ...
            else if (type1.equals(Type.TYPE_STRING))
            {
                ...
                else if (type2.equals(Type.TYPE_CHAR))
                {
                    generator.charToStr ();
                    generator.concatStr();
                }
        ...
        else if (expNode instanceof ASTCastExpression)
        {
            ...
            else if (castType.equals(Type.TYPE_INT))
            {
                        ...
                        expType.equals(Type.TYPE_SHORT) || expType.equals(Type.TYPE_INT))
                {
                    ...
                    generator.d2i();
                else if (expType.equals(Type.TYPE_STRING))
                {
                    generator.strToInt();
                }
                else
            ...
            else if (castType.equals(Type.TYPE_STRING))
            {
                ...
                else if (expType.kind != Type.K_OBJECT)
                    throw new JccException ("" + line + ": invalid type casting");
                return castType;
            }
        ...
        else if (expNode instanceof ASTMemberAccessNode)
        {
            if (expNode.jjtGetChild(0) instanceof ASTSingleId && expNode.jjtGetChild(1) instanceof ASTSingleId)
            {
                ...
                if (ctClass != null)
                {
                    ...
                    if (type.equals(Type.TYPE_INT) || type.equals(Type.TYPE_BYTE))
            ...
            else if (nodeMember instanceof ASTCallNode)
            {
                ...
                if (memberMethod == null)
                    throw new JccException ("" + line + ": '" + sSpec + "' is not a member");
        ...
        else if (expNode instanceof ASTCallNode)
        {
            ...
            String sMethod = ((ASTSingleId)nodeId).str;
            FieldContainer callbackField = locator.getField(sMethod);
            boolean bCallback = false;
            if (callbackField != null && callbackField.type.equals(Type.TYPE_CALLBACK))
                bCallback = true;
            ...
            for (int i = 0; i < arguments.jjtGetNumChildren(); i++)
            {
                argTypes[i] = compileExpression (method, arguments.jjtGetChild(i), null);
            }
            String sSpec = MethodContainer.genSpec(sMethod, argTypes);
            MethodContainer callMethod = locator.getMethod (sSpec);
            if (callMethod == null && !bCallback)
                throw new JccException ("" + line + ": '" + sSpec + "' not found");
            //if (bCallback && !(argTypes.length == 1 && argTypes[0].equals(Type.TYPE_AOBJECT)))
            //    throw new JccException ("" + line + ": invalid callback call");
            generator.fakeModeOff();

            if (bCallback || callMethod.libHost == null)
            {
                generator.loadThis();
            }
            else
            ...
            if (bCallback)
            {
                generator.pushInt(arguments.jjtGetNumChildren());
                generator.newArray(Type.TYPE_OBJECT);
            }
            ...
            for (int i = 0; i < arguments.jjtGetNumChildren(); i++)
            {
                if (bCallback)
                {
                    ...
                    Type tEl = argTypes[i];
                    if (generator instanceof JvmCodeGenerator)
                    {
                        if (tEl.equals(Type.TYPE_INT))
                        {
                            generator.pushInt(1);
                            generator.newArray(Type.TYPE_INT);
                            generator.dup();
                            generator.pushInt(0);
                            compileExpression (method, arguments.jjtGetChild(i), null);
                            generator.iastore();
                        }
                        else if (tEl.equals(Type.TYPE_BYTE) || tEl.equals(Type.TYPE_BOOL))
                        {
                            generator.pushInt(1);
                            generator.newArray(Type.TYPE_BYTE);
                            generator.dup();
                            generator.pushInt(0);
                            compileExpression (method, arguments.jjtGetChild(i), null);
                            generator.bastore();
                        }
                        else if (argTypes[i].equals(Type.TYPE_CHAR))
                        {
                            generator.pushInt(1);
                            generator.newArray(Type.TYPE_CHAR);
                            generator.dup();
                            generator.pushInt(0);
                            compileExpression (method, arguments.jjtGetChild(i), null);
                            generator.castore();
                        }
                        else if (argTypes[i].equals(Type.TYPE_SHORT))
                        {
                            generator.pushInt(1);
                            generator.newArray(Type.TYPE_SHORT);
                            generator.dup();
                            generator.pushInt(0);
                            compileExpression (method, arguments.jjtGetChild(i), null);
                            generator.sastore();
                        }
                        else if (argTypes[i].equals(Type.TYPE_LONG))
                        {
                            generator.pushInt(1);
                            generator.newArray(Type.TYPE_LONG);
                            generator.dup();
                            generator.pushInt(0);
                            compileExpression (method, arguments.jjtGetChild(i), null);
                            generator.lastore();
                        ...
                        {
                            generator.pushInt(1);
                            generator.newArray(Type.TYPE_FLOAT);
                            generator.dup();
                            generator.pushInt(0);
                            compileExpression (method, arguments.jjtGetChild(i), null);
                            generator.fastore();
                        }
                        else if (argTypes[i].equals(Type.TYPE_DOUBLE))
                        {
                            generator.pushInt(1);
                            generator.newArray(Type.TYPE_CHAR);
                            generator.dup();
            ...
            if (bCallback)
            {
                if (!compileExpression (method, nodeId, null).equals(Type.TYPE_CALLBACK))
                    throw new JccException ("" + line + ": invalid callback call");
                generator.callCallback();
        ...
        else if (expNode instanceof ASTSingleId)
        {
            ...
            MethodContainer callbackMethod = locator.getCallback (sId);
            if (callbackMethod != null)
            {
                generator.pushInt(callbackMethod.id);
                return Type.TYPE_CALLBACK;
            }
            FieldContainer field = locator.getField(sId);
            if (field != null)
            {
        ...
        else if (expNode instanceof ASTNewArray)
        {
            ...
            else
            {
                ...
                else if (type.primitive == Type.T_OBJECT)
                    arrayType = new Type (Type.T_OBJECT, nDims);
                else
        ...
        else if (expNode instanceof ASTConstructor)
        {
            Node nodeId = expNode.jjtGetChild(0);
            String sId = "";
            for (int i = 0; i < nodeId.jjtGetNumChildren(); i++)
            {
                sId += ((ASTSingleId)nodeId.jjtGetChild(i)).str;
            }
            ClassContainer classType = locator.getClass (sId);
            if (classType == null)
                throw new JccException ("" + line + ": undefined class '" + sId + "'");
            generator.newObject(classType);
            generator.dup();
            Node nodeArgs = expNode.jjtGetChild(1);
            Type[] argTypes = new Type[nodeArgs.jjtGetNumChildren()];
            for (int i = 0; i < argTypes.length; i++)
            {
                argTypes[i] = compileExpression (method, nodeArgs.jjtGetChild(i), null);
            }
            String sDesc = MethodContainer.genSpec("<init>", argTypes);
            MethodContainer methodConstr = classType.getMethod(sDesc);
            if (methodConstr == null)
                throw new JccException ("" + line + ": '" + sId  + "' has no method '" + sDesc + "'");
            generator.callConstructor(classType, methodConstr);
            return new Type (classType, 0);
        }

        throw new JccException ("" + line + ": can't determine expression type");
    ...
    public Type parseType (Node typeNode) throws JccException
    {
        ...
        else
        {
            ...
            {
                node = node.jjtGetChild(0);
                if (node instanceof ASTCallbackNode)
                {
                    if (arrayDepth != 0)
                        throw new JccException ("" + line + ": invalid type");
                    return Type.TYPE_CALLBACK;
            ...
            else
            {
                ...
                if (className.equals("Object"))
                    return new Type(Type.T_OBJECT, arrayDepth);
