        ...
        for (int i = 0; i < vConst.length; i++)
        {
            ...
            else if (vConst[i] instanceof String)
            {
                ...
                for (int j = 0; j < ((String)vConst[i]).length(); j++)
                    osHeader.writeChar (((String)vConst[i]).charAt(j));
                curPos += ((String)vConst[i]).length()*2;
            }
        }
...
//        }
    ...
    public void newArray(Type type) throws IOException
    {
        ...
        if (type.kind == Type.K_OBJECT)
        {
            ...
            else if (type.equals(Type.TYPE_OBJECT))
            {
                writeByte(-1);
                writeShort(1);
            }
            ...
            {
                writeByte (type.object.libHost.id);
                writeShort (type.object.id);
            }
        }
        else
        {
    ...
    public void pushFloat(float val) throws IOException
    {
        ...
        if (valConst == null)
        {
            ...
            htFloatConst.put(valConst[1], valConst);
        }
        else
        {
            id = ((Integer)valConst[0]).intValue();
    ...
    public void pushDouble(double val) throws IOException
    {
        ...
        opcode (LDC);
        Object[] valConst = (Object[])htDoubleConst.get(new Double(val));
        int id;
        if (valConst == null)
        {
            id = nextConst++;
            ...
            htDoubleConst.put(valConst[1], valConst);
        }
        else
        {
            id = ((Integer)valConst[0]).intValue();
    ...
    {
        opcode (CALL_CALLBACK);
    }

    public void newObject(ClassContainer classContainer) throws IOException
    {
        opcode (NEWOBJECT);
        writeByte (classContainer.libHost.id);
        writeShort (classContainer.id);
    }
