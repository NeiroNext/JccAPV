...
public class JccEdit extends Canvas implements Runnable
{
	...
	void updateCaretPosition()
	{
		...
		{
			inputTranslationX = (bShowLines) ? (iNumbersWidth + 1 - caretLeft) : 1;
		}
...
//                    bSelectingChar = false;
            ...
            else if (currentMode == STATE_MENU)
            {
                ...
                else if(gameAction == Canvas.LEFT || key == '4')
                {
                    showMenu(false);
                }
                else if(gameAction == Canvas.RIGHT || key == '6')
                {
                    if (commandListener != null)
                        commandListener.commandAction((Command)vCommands.elementAt(menuFocus), this);
                    showMenu(false);
                }
                else if (gameAction == Canvas.UP || key == '2')
                {
                    menuFocus--;
                    if (menuFocus < 0)
                        menuFocus = vCommands.size()-1;
                }
                else if (gameAction == Canvas.DOWN || key == '8')
                {
                    menuFocus++;
                    if (menuFocus >= vCommands.size())
                        menuFocus = 0;
                }
                else if (gameAction == Canvas.FIRE || key == '5')
                {
                    if (commandListener != null)
                        commandListener.commandAction((Command)vCommands.elementAt(menuFocus), this);
                    showMenu(false);
                	...
                	if(mode != MODE_MOVE)
                      ...
                      switch(key)
                      {
                        ...
                        case '5': //	Edit line
                            ...
                            selEndLine = iCurLine;
                            int NOWEDIT = 2;
                            commandListener.commandAction((Command)vCommands.elementAt(NOWEDIT), this);
                            bSelecting = false;
                            break;
                        case '0': //	Space
...
//            System.out.println("select char");
	...
	public void paint(Graphics g)
	{
        ...
        for (int iLine = offsetLine; iLine < offsetLine+nLinesMax && iLine < vCurText.size(); iLine++)
        {
            ...
            g.setFont (inputFont);
            if(settings.bSyntax)
            	//g.drawImage(Hightlight.draw((String)vCurText.elementAt(iLine), inputFont), 0, y, Graphics.LEFT | Graphics.TOP);
...
//		System.out.println(inputTranslationX);
		g.translate(- inputTranslationX, 0);
...
//      g.setColor(0xff00ff);
//      g.drawString("fps: "+(50-t1), 1, 1, Graphics.LEFT|Graphics.TOP);
