...
public class SymLocator
{
    ...
    {
        if (method.bCallback)
            htCallbacks.put(method.name, method);
        return curClass.addMethod(method);
    }

    MethodContainer getCallback(String sId)
    {
        return (MethodContainer)htCallbacks.get(sId);
    }
}
